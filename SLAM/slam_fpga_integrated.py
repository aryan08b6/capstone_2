...
class SLAMWithFPGAAcceleration:
        ...
        if self.enable_fpga:
            try:
                ...
                if self.disable_cpu_fallback:
                    raise RuntimeError(f"FPGA-only mode requested but FPGA init failed: {e}")
    ...
    def _init_step_with_pipeline(self, gray: np.ndarray, out: np.ndarray):
        ...
        if self.slam._init_pts2d is None:
            corners = self._process_with_pipeline(gray)
            if corners is not None and len(corners) > 0:
                self.slam._init_pts2d = corners
                self.use_fpga_stats = True
            elif self.disable_cpu_fallback:
                self._put(out, "FPGA-only init failed, retrying…", (10, 28), (60, 220, 220))
                return
            else:
